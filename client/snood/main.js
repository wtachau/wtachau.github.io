...
export default (animate, defaultRender) => {
  ...
  const render = () => {
    ...
    fixedBlocks
      .concat(flashingBlocks)
      .concat(fallingBlocks)
      .concat(movingBlock, pendingBlock, nextBlockInQueue, blockMovingToPending)
      .forEach(b => b?.render());
  };
  ...
  const update = () => {
    ...
    if (movingBlock) {
      ...
      if (collidingBlock || movingBlock.isOffEdge(stepsDown)) {
        const slotToEnter = closestEmptySlot(
          fixedBlocks,
          collidingBlock,
          movingBlock,
          stepsDown
    ...
    const deathBlocks = fixedBlocks.filter(
      b => b.row + stepsDown >= numberRows + numberSteps
    );
